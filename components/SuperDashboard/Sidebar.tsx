...
type Props = {
  activeMenu?: string; // Made optional since it's unused
      ...
      <div>
        <ul className="pt-6 space-y-2 px-4 text-sm font-medium text-[#5F6165]">
          {filteredMenuItems.map((item) => {
            const isActive =
              pathname === item.path || pathname.startsWith(item.path + "/") || (item.subItems?.some((sub) => pathname === sub.path || pathname.startsWith(sub.path + "/")) ?? false);
