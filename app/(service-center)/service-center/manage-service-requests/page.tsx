...
export default function ServiceCenterRequests() {
	const { user } = useAuth();
	...
	const [selectedMechanic, setSelectedMechanic] = useState<{ [key: number]: string }>({});
	const requests = data?.data ?? [];

	const pendingRequests = requests.filter(
		(r) =>
			r.status === "requested" &&
			r.service_request_assignments.some((a: any) => a.status === "pending"),
	);

	const serviceCenter = pendingRequests[0]?.service_request_assignments?.filter(
		(s) => s.service_center.userId === user.id,
	)[0];
					...
					<div className="flex items-center justify-between flex-wrap gap-4">
						<div>
							...
							<p className="text-gray-600">
								Manage and assign incoming service requests to your mechanics
							</p>
						</div>
				...
				{pendingRequests.length === 0 ? (
					<div className="bg-white rounded-2xl shadow-lg p-12 text-center">
						<div className="max-w-md mx-auto">
							...
							<p className="text-gray-600">
								No pending service requests at the moment. New requests will appear
								here.
							</p>
						</div>
					</div>
									...
									<div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
										...
										<div className="flex-1 flex flex-col sm:flex-row gap-3">
											...
											>
												...
												{serviceCenter?.service_center?.mechanics?.map(
													(m: any) => (
											...
											<button
												...
												disabled={!selectedMechanic[req.id]}
												className="px-6 py-3 bg-[rgba(154,225,68,0.8)] hover:bg-[rgba(154,225,68,1)] disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold rounded-xl transition-all duration-200 shadow-md hover:shadow-lg flex items-center justify-center gap-2"
											>
												<svg
													className="w-5 h-5"
													fill="none"
			...
			{selectedRequest && (
				<GoogleMapModal
					...
					center={{
						// lat: 19.845549,
						// lng: 74.045967,
						lat: serviceCenter ? serviceCenter.service_center?.latitude : 1,
						lng: serviceCenter ? serviceCenter.service_center?.longitude : 1,
					}}
				/>
			)}
		</div>
