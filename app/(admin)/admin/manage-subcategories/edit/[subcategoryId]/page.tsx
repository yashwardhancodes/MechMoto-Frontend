...
import { useUpdateSubcategoryMutation, useGetSubcategoryQuery } from "@/lib/redux/api/subCategoriesApi";
...
const UpdateSubcategory: React.FC = () => {
  ...
  useEffect(() => {
    if (subcategory) {
      setFormData({
        ...
        imgSrc: subcategory.data.img_src || "",
        categoryId: subcategory.data.categoryId ? subcategory.data.categoryId.toString() : "",
      });
      setPreviewUrl(subcategory.data.img_src || null);
    }
  }, [subcategory]);
  ...
  useEffect(() => {
    if (file) {
      ...
      setPreviewUrl(formData.imgSrc || null);
    }
  }, [file, formData.imgSrc]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const result = await updateSubcategory({ id: subcategoryId, ...parsedData }).unwrap();
      console.log("✅ API Response:", result);

      if (result?.success) {
        toast.success("Subcategory updated successfully! Redirecting...");
        router.push("/admin/manage-subcategories");
      } else {
  ...
  if (subcategoryError) {
    return <div>Error loading subcategory. Please try again.</div>;
  }
        ...
        <div className="space-y-9">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              {errors["imgSrc"] && (
                <p className="text-red-500 text-sm mt-1">{errors["imgSrc"]}</p>
              )}
            </div>
