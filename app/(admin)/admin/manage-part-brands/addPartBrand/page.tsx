...
import { useCreatePartBrandMutation } from "@/lib/redux/api/partBrandApi";
...
const AddPartBrand: React.FC = () => {
    const [formData, setFormData] = useState<FormData>({ name: "" });   
    const [errors, setErrors] = useState<{ [key: string]: string }>({});
    const [addPartBrand, { isLoading }] = useCreatePartBrandMutation();
    ...
    const handleSubmit = async () => {
        try {
            ...
            const result = await addPartBrand(parsedData).unwrap();
            if (result?.success) {
                toast.success("Part Brand added successfully!");
                setFormData({ name: "" });
                router.push("/admin/manage-part-brands?refresh=true");
            } else {
                toast.error("Failed to add Part Brand.");
            }
        } catch (err: unknown) {
    ...
    return (
        <div className="h-[calc(100vh-140px)] overflow-y-auto bg-white shadow-sm py-16 px-12">
            <div className="mx-auto space-y-6">
                <h2 className="text-xl font-semibold">Add Part Brand</h2>
                <div className="flex gap-4">
                    ...
                    <div>
                        ...
                        >
                            {isLoading ? "Adding..." : "Add Part Brand"}
                        </button>
                    </div>
                </div>
            </div>
