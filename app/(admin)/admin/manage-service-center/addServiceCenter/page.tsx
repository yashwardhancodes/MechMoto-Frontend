...
import { useCreateServiceCenterMutation } from "@/lib/redux/api/serviceCenterApi";
...
const LocationPicker: React.FC<{
  ...
  useMapEvents({
    click(e) {
      ...
      toast.success(`Selected: ${e.latlng.lat.toFixed(5)}, ${e.latlng.lng.toFixed(5)}`);
    },
  });
...
const AddServiceCenter: React.FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const result = await addServiceCenter(parsedData).unwrap();
    ...
    <div className="h-[calc(100vh-170px)] overflow-y-auto bg-white shadow-sm py-16 px-4">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex justify-end mt-3">
          ...
          >
            {isLoading ? "Adding..." : "Add Service Center"}
          </button>
        </div>
      </div>
    </div>
