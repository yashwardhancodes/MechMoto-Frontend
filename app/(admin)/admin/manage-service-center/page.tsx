...
import { useGetAllServiceCentersQuery ,useDeleteServiceCenterMutation} from "@/lib/redux/api/serviceCenterApi";
...
export default function ManageServiceCenters() {
  const { data, isLoading, isError } = useGetAllServiceCentersQuery({});
  ...
  const columns: TableColumn[] = [
    {
      ...
      render: (value) => (
        <div className="flex flex-col">
          ...
          <span className="text-xs text-gray-500">{value.city}, {value.state}</span>
        </div>
      )
    },
    {
