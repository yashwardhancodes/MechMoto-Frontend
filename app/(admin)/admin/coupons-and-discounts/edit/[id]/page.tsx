...
const UpdateCoupon: React.FC = () => {
	...
	useEffect(() => {
		if (coupon?.data) {
			setFormData({
				...
				discount_type: coupon.data.discount_type || "percentage",
				discount_value: coupon.data.discount_value?.toString() || "",
				max_discount: coupon.data.max_discount?.toString() || "",
				min_order_amount: coupon.data.min_order_amount?.toString() || "",
				valid_from: coupon.data.valid_from
					? new Date(coupon.data.valid_from).toISOString().slice(0, 16)
					: "",
				valid_until: coupon.data.valid_until
					? new Date(coupon.data.valid_until).toISOString().slice(0, 16)
					: "",
				usage_limit: coupon.data.usage_limit?.toString() || "",
				is_active: coupon.data.is_active ? "true" : "false",
			});
		}
	}, [coupon]);
	...
	const handleSubmit = async () => {
		try {
			setErrors({});
			const parsedData = updateCouponSchema.parse({
				...formData,
	...
	if (isCouponLoading) return <div>Loading coupon data...</div>;
	if (couponError) return <div>Error loading coupon data</div>;
