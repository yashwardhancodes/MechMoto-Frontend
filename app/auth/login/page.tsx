	...
	const handleSubmit = async () => {
		try {
			...
			if (result.success) {
				...
				// ✅ Handle storage based on rememberMe after Redux updates state
				const authData = {
					user: result.data.user, // Adjust based on your API response structure
				...
				if (rememberMe) {
					...
					// Session-only
					sessionStorage.setItem(storageKey, JSON.stringify(authData));
					// Clear localStorage
					...
					<div className="space-y-4 px-6">
						...
						<div className="flex justify-between items-center text-sm text-[#1C274C]">
							<label className="flex items-center gap-2 cursor-pointer">
								<input
									type="checkbox"
									checked={rememberMe} // ✅ Bind state
									onChange={handleRememberMeChange} // ✅ Add handler
									className="accent-[#6BDE23] size-4"
								/>
