...
const UpdatePart: React.FC = () => {
  ...
  const { data: discounts, isLoading: isDiscountsLoading, error: discountsError } = useGetAllPartBrandsQuery({});
  ...
  useEffect(() => {
    if (part?.data) {
      ...
      setPreviewUrls(part.data.imageUrls || []);
    }
  }, [part]);
      ...
      <div className="max-w-5xl mx-auto">
        <div className="space-y-9">
          ...
          <div className="relative">
            ...
            {dropdownOpen.vehicle && !isVehiclesLoading && !vehiclesError && Array.isArray(vehicles?.data) && vehicles.data.length > 0 && (
              <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                {vehicles.data.map((vehicle: Vehicle) => (
                  ...
                  >
                    {vehicle.car_make?.name || ""} {vehicle.model_line || ""} {vehicle.modification ? `(${vehicle.modification})` : ""} {vehicle.engine_type?.name ? `- ${vehicle.engine_type.name}` : ""} [{vehicle.production_year || ""}]
                  </button>
                ))}
              </div>
            )}
